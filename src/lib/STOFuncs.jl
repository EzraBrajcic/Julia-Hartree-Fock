...
function CoulombInt(a::STO, b::STO, c::STO, d::STO)
    # Represents the integral (ab|cd) = ∫∫φ†_a(r1)φ_b(r1)1/|r1-r2|φ†_c(r2)φ_d(r2) dr1dr2 (also represented as <φ_a φ_b|1/|r1-r2||φ_c φ_d>) such that the radial coordinates follow the form of (12|12). The 1/|r1-r2| term can be transformed into a Laplace expansion of the form ∑_(L=0)^(∞) (4pi/(2L + 1)) * ∑_(M=-L)^(L) r_<^L/r_>^(L+1) * Y_Lω(Ω1)Y*_Lω(Ω2) where r_< is the smaller of r1 and r2 and r_> is the larger of r1 and r2. The angular portion of the integral can then be solved using Gaunt coefficients which are products of 3j-Wigner symbols, while the radial portion can be solved using incomplete gamma functions.
    result::Float64 = 0.0
